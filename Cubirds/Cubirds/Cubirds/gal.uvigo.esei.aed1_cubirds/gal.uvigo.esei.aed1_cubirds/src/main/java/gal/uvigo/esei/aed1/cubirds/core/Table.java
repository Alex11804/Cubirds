...
public class Table {
    ...
    /**
     * Constructor: Se crea un array vacío de 4 filas y 3 columnas 
     */ 
    public Table() {
    ...
    public void setCard(int row, int column, Card card){
        if(row < 0 || row > 3){
            throw new IndexOutOfBoundsException("La fila debe ser 0 - 3");
        }
        if(cardsTable[row] == null){
            cardsTable[row] = new LinkedList<>();
        }
        cardsTable[row].add(column, card);
    }
            ...
            while(column < 3){                 //n columnas de cartas 
                ...
                if(card != null){
                    if(!sameTypeOfBird(row, card)){         //Si el tipo de pajaro de esa carta es distinto al anterior (en caso de que haya anterior, si no ,se ejecutaría igualmente)
                        setCard(row, column, card);         //Se coloca la carta en la mesa 
